    ...
    <template v-else>
      <div class="account_info">
        ...
        <table class="account_info_wrap">
          <thead>
          <tr><th style="text-align: left;font-size: 20px">平台合作方账户基本资料</th></tr>
          </thead>
          ...
          <tr>
            ...
            <td>平台合作方账户（善林商务）</td>
            <td>账户编号：</td>
            <td>SYS_GENERATE_008</td>
          </tr>
          <tr>
            <td>账户说明：</td>
            <td colspan="3" >平台为合作方分配一个平台编号，相应的存管银行为合作方账户</td>
          </tr>
          <tr>
            <td>绑定对公账户：</td>
            <td v-show="isBindingCard">尚未绑定银行对公账户<span class="binding" @click="bindBankCard">绑定</span></td>
            <td v-show="!isBindingCard">您已绑定银行对公账户：尾号{{tailNum}}<span class="binding" @click="unbind('SYS_GENERATE_008')">解绑</span></td>
          </tr>
          </tbody>
      ...
      <bill-download></bill-download>
      <account-record
        recordTitle="平台合作账户资金记录"
        :tableData="tableData"
        @query="queryRecord"
      ></account-record>
    </template>
...
<script>
  ...
  import BillDownload from '../common/BillDownload.vue'
  ...
  export default {
    ...
    methods:{
      ...
      bindBankCard(){
        ...
        this.$refs.bingCard.form.whichClick = "SYS_GENERATE_008";
      },
    ...
    mounted(){
      this.$http.post("/api/order/selectDownOrder",addSign({
        "platformUserNo":"SYS_GENERATE_008",
        "orderType":"4001",
        "orderSource":"1",
        "orgCode":"xwbank"
      })).then((res)=>{
