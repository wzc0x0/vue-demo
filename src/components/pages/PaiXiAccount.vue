    ...
    <template v-else>
      <div class="account_info">
        <table class="account_info_wrap">
          <thead>
          <tr><th style="text-align: left;font-size: 20px">平台派息账户基本资料</th></tr>
          </thead>
          ...
          <tr>
            <td>账户说明：</td>
            <td colspan="3" >用于平台为用户发放投资人补偿利息或借款人换款时将利息还到该账户</td>
          </tr>
          <tr>
            <td>绑定对公账户：</td>
            <td v-show="isBindingCard">尚未绑定银行对公账户<span class="binding" @click="bindBankCard">绑定</span></td>
            <td v-show="!isBindingCard">您已绑定银行对公账户：尾号{{tailNum}}<span class="binding" @click="unbind('SYS_GENERATE_006')">解绑</span></td>
          </tr>
          </tbody>
  ...
  export default {
    ...
    methods:{
      ...
      bindBankCard(){
        ...
        this.$refs.bingCard.form.whichClick = "SYS_GENERATE_006";
      },
