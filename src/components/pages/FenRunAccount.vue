      ...
      <div class="account_info">
        <table class="account_info_wrap">
          ...
          <tr>
            ...
            <td>分润账户</td>
            <td>账户编号：</td>
            <td>SYS_GENERATE_003</td>
          </tr>
          <tr>
            <td>账户说明：</td>
            <td colspan="3" >平台现将待分润总金额收取到该账户，方便事后分润或者多级分润</td>
          </tr>
          <tr>
            <td>绑定对公账户：</td>
            <td v-show="isBindingCard">尚未绑定银行对公账户<span class="binding" @click="bindBankCard">绑定</span></td>
            <td v-show="!isBindingCard">您已绑定银行对公账户：尾号{{tailNum}}<span class="binding" @click="unbind('SYS_GENERATE_003')">解绑</span></td>
          </tr>
          </tbody>
  ...
  export default {
    ...
    mounted(){
      this.$http.post("/api/order/selectDownOrder",addSign({
        "platformUserNo":"SYS_GENERATE_003",
        "orderType":"4001",
        "orderSource":"1",
        "orgCode":"xwbank"
      })).then((res)=>{
