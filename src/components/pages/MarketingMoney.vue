    ...
    <template v-else>
      <div class="account_info">
        <table class="account_info_wrap">
          <thead>
          <tr><th style="text-align: left;font-size: 20px">营销款账户基本资料</th></tr>
          </thead>
          ...
          <tr>
            ...
            <td>营销款账户</td>
            <td>账户编号：</td>
            <td>SYS_GENERATE_005</td>
          </tr>
          <tr>
            <td>账户说明：</td>
            <td colspan="3" >用于平台发放营销款（现金红包、活动红包等）</td>
          </tr>
          <tr>
            <td>绑定对公账户：</td>
            <td v-show="isBindingCard">尚未绑定银行对公账户<span class="binding" @click="bindBankCard">绑定</span></td>
            <td v-show="!isBindingCard">您已绑定银行对公账户：尾号{{tailNum}}<span class="binding" @click="unbind('SYS_GENERATE_005')">解绑</span></td>
          </tr>
          </tbody>
  ...
  export default {
    ...
    methods:{
      ...
      bindBankCard(){
        ...
        this.$refs.bingCard.form.whichClick = "SYS_GENERATE_005";
      },
